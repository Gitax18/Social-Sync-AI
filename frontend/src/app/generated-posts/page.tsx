...
export default function GeneratedPosts() {
  ...
  const handleRegenerate = async (platform: string) => {
    ...
    const newContent = await AiApi.generate(platform, {
      description: content,
        ...
        {Object.keys(posts).length === 0 ? (
            ...
            <CardContent className="py-8">
              <div className="text-center">
                <p className="text-muted-foreground">
                  No platforms selected. Please go back and select at least one
                  platform.
                </p>
              </div>
            </CardContent>
          ...
          <div className=" px-5 gap-5  flex justify-center flex-wrap ">
            {Object.entries(posts).map(([platform, post]) => (
              <Card
                key={platform}
                className={`overflow-hidden max-w-[355px] w-full`}
              >
                ...
                <CardContent className="pt-4">
                  <textarea
                    className="whitespace-pre-wrap min-h-[100px] w-full "
                    value={post.content ?? ""}
                    readOnly
                  />
                </CardContent>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter className="sm:justify-between">
            <Button
              type="button"
              variant="default"
              onClick={() => handleSave(selectedPost[1])}
            >
              Save
            </Button>
